/* LongMultGV.c Bayesian Nonparametric Long Multivariate Regression
                ...
                int *G1, double *DPparams)
{
    ...
    FILE *out_file1, *out_file2, *out_file3, *out_file4, *out_file5, *out_file6,
         ...
         *out_file19, *out_file20, *out_file21, *out_file22, *out_file23, *out_file24,
         *out_file25, *out_file26, *out_file27;
    ...
    out_file26 = fopen(path_name, "a");
    snprintf(path_name, MAX_PATH, "%s%s", *WorkingDir, "BNSP.tune.txt");
    out_file27 = fopen(path_name, "a");
...
*/
    ...
    for (h = 0; h < G-1; h++){
        nmb += nmembersV[h];
        Vdp[h] = gsl_ran_beta(r, (double) nmembersV[h]+1.0, (double) p-nmb+alphaDP);
        if (h == 0)
            w[h] = Vdp[h];
        else 
		...
		for (l = 0; l < p; l++){
		    for (j = 0; j < NG; j++){
	            ...
	            for (block = 0; block < nBlocks; block++){
                    ...
                    //fprintf(out_file26, "%s %i %f ","gamma: ", sw, Acp);
...
//Rprintf("%s %f %f %f %f %f %f %f \n","ceta 1: ",cetahat,elPrime,SPC,Sprime,SDprime,Q2,sigma2zk[0]);
	    ...
	    unifRV = gsl_ran_flat(r,0.0,1.0);
	    //fprintf(out_file26, "%s %i %f ","cbeta: ", sw, Acp);
        if (Acp > unifRV || sw < 5){
            ceta = cetaP;
            SPC = SPP;
...
//sw,Acp,unifRV,SPC,SPP,ceta,cetahat,cetaP,Q2,alphaeta,betaeta,Ngamma,tuneCb[0],elDPrime);
        ...
        for (l = 0; l < p; l++){
            for (j = 0; j < ND; j++){
		        if ((sw % batchL)==0 && WB > 0){ 
	                ...
	                fprintf(out_file27,"%f ",tuneAlpha[ND*l+j]);
                }
		        for (k = 0; k < vecLD[j]; k++)
                ...
                for (block = 0; block < nBlocks; block++){
	                ...
	                //fprintf(out_file26, "%s %i %f ","delta: ", sw, Acp);
	                if (Acp > unifRV){
		...
		for (k = 0; k < p; k++){
            if ((sw % batchL)==0 && WB > 0){
	            ...
	            fprintf(out_file27,"%f ",tuneSigma2[k]);
            }
	        ...
	        //fprintf(out_file26, "%s %i %f ","sigma2: ", sw, Acp);
            if (Acp > unifRV){
                sigma2zk[k] = sigma2P;	            	            
	            for (i = 0; i < m; i++)
                    for (t = 0; t < niVec[i]; t++)
        ...
        for (k = 0; k < p; k++){
            ...
            if (HNca[k]==0){
                ...
                //fprintf(out_file26, "%s %i %f ","calpha: ", sw, Acp);      
	            unifRV = gsl_ran_flat(r,0.0,1.0);
                ...
                for (j = 0; j < NK; j++){		        		            		            		            
                    ...
                    for (block = 0; block < nBlocks; block++){                                            
                        ...
                        */
                        ...
                        unifRV = gsl_ran_flat(r,0.0,1.0);
                        //fprintf(out_file26, "%s %i %f ","ksi & psi : ", sw, Acp);                                               
	                    if (Acp > unifRV){               					
            ...
            for (l = 0; l < p; l++){
                ...
                if (HNcpsi[k*p+l]==0){
                    ...
                    if ((sw % batchL)==0 && WB > 0){
	                    ...
	                    fprintf(out_file27,"%f ",tuneCpsi[k*p+l]);
                    }
                    phi2G = cpsiParams[move++];
	                ...
	                unifRV = gsl_ran_flat(r,0.0,1.0);
                    //fprintf(out_file26, "%s %i %f ","cpsi: ", sw, Acp);
                    if (Acp > unifRV){
                        cPsi[k*p+l] = cPsiP;
		...
		for (t = 0; t < LUT; t++){	        
            ...
            Acp = exp(logAcp);
            unifRV = gsl_ran_flat(r,0.0,1.0);                        
            //fprintf(out_file26, "%s %i %f ","Rt: ", sw, Acp);          
            if (Acp > unifRV || sw < 5){
            ...
            for (i = 0; i < G; i++){
                ...
                for (l = 0; l < p; l++){
                    if (l != k){
                        ...
                        for (t = 0; t < LUT; t++){
                            pdh[i][k] *= gsl_ran_gaussian_pdf(theta[pos+t*d]-ZetaStore[compAllocP[pos]][t],sqrt(sigma2t[t]));                    
                            if (1==0) fprintf(out_file26,"%i %i %i %i %i %i %i %f \n",sw,k,i,l,x,y,pos,pdh[i][k]);
						}
                    }
                }                                    
		    }
        ...
        for (h = 0; h < H; h++){
            if (nmembers[h]>0){
                ...
                if ((sw - burn) >= 0 && 1==0){
                    move = 0; 
                    fprintf(out_file26, "%f ", gsl_vector_get(&subMeanEta.vector,move++));
                    for (j = 0; j < LGc; j++)
                        if (gammaCor[h][j]==1) fprintf(out_file26, "%f ", gsl_vector_get(&subMeanEta.vector,move++)); else fprintf(out_file26, "%f ", 0.0);                
                    fprintf (out_file26, "\n");	       
	                for (j = 0; j < (LUT*d); j++)
                        fprintf(out_file26, "%f ", theta[j]);
                    fprintf (out_file26, "\n");
                    for (j = 0; j < (LUT*d); j++)
                        fprintf(out_file26, "%f ", sqResC[j]);
                    fprintf (out_file26, "\n");
                }
        ...
        for (j = 0; j < NDc; j++){
		    if ((sw % batchL)==0 && WB > 0){ 
	            ...
	            fprintf(out_file27,"%f ",tuneOmega[j]);
            }            
            ...
            for (block = 0; block < nBlocks; block++){  
                ...
                unifRV = gsl_ran_flat(r,0.0,1.0);    
                //fprintf(out_file26, "%s %i %f %f %f %f %f %f %f %f %f %f %i %i \n","d-o: ", sw, Acp,
		...
		if (HNsigcor[0]==0){ 
            ...
            if ((sw % batchL)==0 && WB > 0){ 
	            ...
	            fprintf(out_file27,"%f ",tuneSigma2R[0]);
            }            
	        ...
	        unifRV = gsl_ran_flat(r,0.0,1.0);
	        //fprintf(out_file26, "%s %i %f ","sigma2cor: ", sw, Acp);
            if (Acp > unifRV){
                sigma2cor = sigma2P;
        ...
        if ((sw % batchL)==0 && WB > 0){ 
	        ...
	        fprintf(out_file27,"%f ",tuneCbCor[0]);
        }     
	    ...
	    unifRV = gsl_ran_flat(r,0.0,1.0);
	    //fprintf(out_file26, "%s %i %f ","ceta cor: ", sw, Acp);
        if (Acp > unifRV){
            cetaCor = cetaP;
        ...
        if (HNco[0]==0){ 
		    ...
		    //fprintf(out_file26, "%s %i %f %f %i %f %f \n","comega: ", sw, alphaG, betaG, NdeltaCor, QFC, comega);
		    if ((sw % batchL)==0 && WB > 0) fprintf(out_file27,"\n");
		}else if (HNco[0]==1){
			phi2G=comegaParams[0];
            if ((sw % batchL)==0 && WB > 0){ 
        ...
        if (((sw - burn) >= 0) && (((sw - burn ) % thin) == 0) && (WF == 1)){
			...
			for (i = 0; i < p; i++)
                fprintf(out_file23, "%i ", compAllocV[i]);
            fprintf(out_file23, "\n");
            
            for (h = 0; h < G; h++)
                fprintf(out_file24, "%i ", nmembersV[h]);
            fprintf(out_file24, "\n");
    ...
    fclose(out_file22); fclose(out_file23); fclose(out_file24);
    fclose(out_file25); fclose(out_file26); fclose(out_file27); 

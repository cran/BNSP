/* LongMultWish.c Bayesian Nonparametric Long Multivariate Regression
...
void   longmulw(int *seed1, char **WorkingDir, int *WF1, 
                int *sbtmpN, 
                ...
                double *tautol, int *FT, double *dev,
...
//sw,Acp,unifRV,SPC,SPP,ceta,cetahat,cetaP,Q2,alphaeta,betaeta,Ngamma,tuneCb[0],elDPrime);
		...
		for (t = 0; t < LUT; t++){	        
	        ...
	        if ((sw % batchL)==0 && WB > 0){ 
	            ...
	            fprintf(out_file21,"%s %f %i %f %f \n","tuneR",WB,t,acceptRt[t],tuneR[t]);
		        acceptRt[t] = 0.0;

/* multT.c Bayesian Nonparametric Multivariate Regression
...
extern int MultiNormalPDF(unsigned dim, const double *x, void *parameters, unsigned fdim, double *fval);
...
void multT(int *seed1, char **WorkingDir, 
           int *sbt1,
           ...
           int *LASTksi, double *LASTpsifi2cpsi, int *isSin)
{
    ...
    snprintf(path_name, MAX_PATH, "%s%s", *WorkingDir, "BNSP.test.txt");
    ...
    for (sw = 0; sw < sweeps; sw++){
        ...
        for (i = 0; i < m; i++){
            for (j = 0; j < p; j++){
                ...
                if (i==-99) fprintf(out_file17,"%f %f | %f %f %f %f | %f %f | %f %f | %f %f | %f %f | %f %f %f \n",			                        
			                        omegaij[i][j], Qa,
			                        residuals[i][j], sumlnj, sigma2ij[i][j], Qb,
			                        gsl_matrix_get(RtCinv,j,j), Qc,
			                        mode, curv,
			                        p1, p2,
			                        exp(Qc*(UijP-U[i][j])+Qb*(sqrt(UijP)-sqrt(U[i][j]))+Qa*(log(UijP)-log(U[i][j]))),
                                    gsl_ran_gamma_pdf(U[i][j],p1,p2)/gsl_ran_gamma_pdf(UijP,p1,p2),
                                    U[i][j],UijP,
	            ...
	            //if (i > 6500) Rprintf("%s %i %i %i %f %f %f \n","U Y:",i,j,p,U[i][j],sigma2ij[i][j],Ytilde[i*p+j]);     
			}
		}		
...
//print_matrix(St); 
        ...
        for (l = 0; l < p; l++){
            for (j = 0; j < ND; j++){
				//Rprintf("%i %i %i %s \n",sw,l,j,"delta & alpha");
				//Rprintf("%s %i %i | %i %i %i | %i %i | %f %f %f \n","vec cusum cusum...",
				//l,j,vecLD[j],cusumVecLD[j],cusumVecLD[j+1],isDz[j],ND,tuneAlpha[ND*l+j],csigma[l][j],dsigma[l][j]);
		        if ((sw % batchL)==0 && WB > 0){
                ...
                for (block = 0; block < nBlocks; block++){
                    ...
                    //fprintf(out_file17,"%s %i %i %i | %i %f %i %i %i \n","DP AH AP AC: ",
                    //                    sw,l,j,
                    //                    block,nBlocks,vecLD[j],cusumVecLD[j],NPJ);
                                        
                    //for (k = 0; k < vecLD[j]; k++)
					//	fprintf(out_file17,"%i ",vecDeltaP[j][k]);                    
                    //fprintf(out_file17,"\n");
                    
                    for (k = 0; k < NPJ; k++) 
						fprintf(out_file17,"%s %i %i %i | %f %f %f ","alphaHat, D, tune:",sw,l,j,
						    gsl_vector_get(alphaHat,k), 
						    gsl_matrix_get(&subD.matrix,k,k),
						    tuneAlpha[ND*l+j]);                    
                    if (NPJ > 0) fprintf(out_file17,"\n");
                    
                    for (k = 0; k < vecLD[j]; k++)
						fprintf(out_file17,"%s %f ","alpha",alpha[l][cusumVecLD[j]+k]);                    
                    fprintf(out_file17,"\n");
                    
                    for (k = 0; k < vecLD[j]; k++)
						fprintf(out_file17,"%s %f ","proposed",alphaPD[cusumVecLD[j]+k]);                    
                    fprintf(out_file17,"\n");                                                
				    }
                    ...
                    for (i = 0; i < m; i++){
                        ...
                        //if (!(LPV[i][l] == LPVP[i][l])) Rprintf("%s %i %i %i \n","new:",sw,i,l);
                        //if (!(sigma2ijP[i][l] == sigma2ij[i][l])) Rprintf("%s %i %i %i \n","new:",sw,i,l);
                        //if (!(YtildeP[i*p+l] == Ytilde[i*p+l])) Rprintf("%s %i %i %i \n","new:",sw,i,l);
                    }
                    ...
                    if (1==0){
					    for (i = 0; i < m; i++)
						    fprintf(out_file17,"%s %i %i %i %f %f \n","s-t:",
						            i,0,p,YtildeP[i*p+0],Ytilde[i*p+0]);
						
						for (i = 0; i < m; i++)
						    fprintf(out_file17,"%s %i %i %i %f %f \n","s-t:",
						            i,1,p,YtildeP[i*p+1],Ytilde[i*p+1]);	
                    ...
                    for (k = 0; k < NPJ; k++)
						fprintf(out_file17,"%s %f %f %f ","second:",
						    gsl_vector_get(alphaHat,k), 
						    gsl_matrix_get(&subD.matrix,k,k),
						    tuneAlpha[ND*l+j]);                    
                    if (NPJ > 0) fprintf(out_file17,"\n");
                    ...
                    for (k = 0; k < vecLD[j]; k++)
						fprintf(out_file17,"%f ",alpha[l][cusumVecLD[j]+k]);                    
                    fprintf(out_file17,"\n");
                    
                    for (k = 0; k < vecLD[j]; k++)
						fprintf(out_file17,"%f ",alphaPD[cusumVecLD[j]+k]);                    
                    fprintf(out_file17,"\n");                                                
				    }
	                ...
	                if (1==0 && isDz[j] == 1){
	                    fprintf(out_file17,"%s %i %i %i %i %i %i | %f %f | %f %f %f %f %f \n","Acp: ",
                        sw,l,j,block,NCJ,NPJ,
                        Acp,unifRV,
                        detR, (-SPP+SPC)/2, logMVNormC-logMVNormP, QFD/(2*calpha[l]), pow(2*M_PI*calpha[l],0.5*(NCJ-NPJ)));
                        fprintf(out_file17,"\n"); 
				    }                                       
		...
		for (k = 0; k < p; k++){
	        ...
	        //fprintf(out_file17,"%s %i %i | %f | %f %f | %f | %f %f %f %f %f\n","sigma2: ",
	        //sw,k,
	        //Acp,
	        //sigma2zk[k],sigma2RP,
	        //detR+(SPC-SPP)/2+priorLogR,
	        //priorLogR,(SPC-SPP)/2,SPC,SPP,detR);
...
//sw,Acp,unifRV,sigma2zk[0]*SPC,sigma2zk[0]*SPP,ceta,cetahat,cetaP,sigma2zk[0]*Q2,alphaeta,betaeta,Ngamma,tuneCb[0],elDPrime);
        ...
        if (p > 1){
            ...
            //fprintf(out_file12, "%s %f %f %f \n", "St2 :", gsl_matrix_get(St2,0,0),gsl_matrix_get(St2,0,1),gsl_matrix_get(St2,1,1));

            // - 11 - Update R
            //Rprintf("%i %s \n",sw,"R");
	        if ((sw % batchL)==0 && WB > 0){
	            //fprintf(out_file12, "%i %f %f \n", sw, acceptR, tuneR[0]);
...
if (1==0){
fprintf(out_file12, "%i %.2f | %f %f %f | %f %f %f | %f %f \n%f %f %f %f %f %f %f\n",
sw, tuneR[0],
...
logPropDRC,logPropDRP,priorLogR,detR,SPP,SPC,Acp);
            ...
            // - 12 - Update Theta
            //Rprintf("%i %s \n",sw,"Theta");
            vecTheta = gsl_vector_view_array(theta,d);
            for (k = 0; k < d; k++)
                grt[k] = (FisherTr(Rvec[k],FT[0])/tau2 + muR/sigma2R) / (1/tau2 + 1/sigma2R);
            Ar = gsl_vector_view_array(grt,d);
            gsl_matrix_set_identity(A);
            gsl_matrix_scale(A,1/(1/tau2 + 1/sigma2R));
            s = gsl_ran_flat(r,1.0,100000);
            sampleMN(s,d,&vecTheta.vector,&Ar.vector,A,tol);
            sumtheta = 0.0;
            for (k = 0; k < d; k++)
                sumtheta += theta[k];
            //fprintf(out_file12,"%s %i %f %f %f \n","theta: ",sw,theta[0],sumtheta,grt[0]);

            // - 13 - Update muR
            //Rprintf("%i %s \n",sw,"muR");
            temp = 1/(d/sigma2R+1/varmuR);
            muR = temp*(sumtheta/sigma2R) + gsl_ran_gaussian(r,sqrt(temp));
            sstheta = 0.0;
            for (k = 0; k < d; k++)
                sstheta += pow(theta[k]-muR,2);
            //fprintf(out_file12,"%s %i %f %f %f %f \n","muR: ",sw,temp,muR,sstheta,varmuR);
        ...
        if (((sw - burn) >= 0) && (((sw - burn ) % thin) == 0)){
            ...
            if (compDev==1){                                   				
                ...
                for (i = 0; i < m; i++){  					
                    ...
                    //Rprintf("%i %i %i %f %f %f %f %f %f %f \n",sw,i,j,params[0],params[1],params[2],params[3],params[4],params[5],params[6]);
            ...
            if (p > 1){
                ...
                if (sw==LASTsw[0]){
                    for (k = 0; k < p; k++)
                        for (l = 0; l < p; l++)
                            fprintf(out_file12, "%f ", gsl_matrix_get(DtC,k,l));
                    fprintf(out_file12, "\n");

                    for (k = 0; k < p; k++)
                        for (l = 0; l < p; l++)
                            fprintf(out_file12, "%f ", gsl_matrix_get(EtC,k,l));
                    fprintf(out_file12, "\n");
			    }
                fprintf(out_file7, "%0.10f \n", muR);
            ...
            for (k = 0; k < p; k++)
                for (j = 0; j < LC; j++)
                    fprintf(out_file13, "%f ", psi[k][j]);
            fprintf(out_file13, "\n");

            for (k = 0; k < p; k++)
                for (j = 0; j < LC; j++)
                    fprintf(out_file14, "%i ", ksi[k][j]);
            fprintf(out_file14, "\n");
            
            for (j = 0; j < p; j++)
                fprintf(out_file15, "%f ", fi2[j]);
            fprintf(out_file15, "\n");
            
            for (j = 0; j < p; j++)
                fprintf(out_file16, "%f ", cpsi[j]);
            fprintf(out_file16, "\n");            
        }
